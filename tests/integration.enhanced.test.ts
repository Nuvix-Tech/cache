...
describe("Enhanced Integration Tests", () => {
  ...
  describe("Real-world Usage Scenarios", () => {
    skipIfNoRedis()("should handle user session management", async () => {
      ...
      // Retrieve session
      const retrievedSession = await redisCache.get(`session:${sessionData.userId}`);
      ...
      // Verify update
      const finalSession = await redisCache.get(`session:${sessionData.userId}`) as any;
      expect(finalSession?.lastActivity).toBeDefined();

      // Clean up all active sessions
      await redisCache.flushByTags(['active-users']);
      const cleanedSession = await redisCache.get(`session:${sessionData.userId}`);
    ...
    skipIfNoRedis()("should handle product catalog caching", async () => {
      ...
      const cachedProducts = await redisCache.mget(productKeys, { namespace: 'ecommerce' });
      
      expect(cachedProducts).toHaveLength(3);
      expect(cachedProducts[0]).toEqual(products[0]);

      // Get category data
      const electronicsData = await redisCache.get('category:electronics', { namespace: 'ecommerce' });
      ...
      const updatedElectronics = await redisCache.get('category:electronics', { namespace: 'ecommerce' });
      expect(updatedElectronics).toBeNull();

      // Products should still exist (different tags)
      const stillCachedProduct = await redisCache.get('product:prod1', { namespace: 'ecommerce' });
    ...
    skipIfNoRedis()("should handle analytics and counters", async () => {
      ...
      const storedAnalytics = await redisCache.get('analytics:daily:2025-06-17');
    ...
    skipIfNoRedis()("should handle configuration management", async () => {
      ...
      const featureFlags = await redisCache.get('app:feature_flags', { namespace: 'configuration' }) as any;
      expect(featureFlags?.newDashboard).toBe(true);

      // Bulk retrieve configurations
      const configKeys = Object.keys(configs);
      const allConfigs = await redisCache.mget(configKeys, { namespace: 'configuration' });
      
      expect(allConfigs).toHaveLength(3);
      expect(allConfigs[0]).toEqual(configs['app:feature_flags']);

      // Update feature flag
      const updatedFlags = { ...configs['app:feature_flags'], advancedSearch: true };
      ...
      const newFlags = await redisCache.get('app:feature_flags', { namespace: 'configuration' }) as any;
      expect(newFlags?.advancedSearch).toBe(true);

      // Clear all configuration cache
      await redisCache.flushNamespace('configuration');
      
      const clearedConfig = await redisCache.get('app:settings', { namespace: 'configuration' });
      expect(clearedConfig).toBeNull();
    });
  });

  describe("Performance and Stress Testing", () => {
    skipIfNoRedis()("should handle large batch operations efficiently", async () => {
      const batchSize = 100;
      ...
      const results = await redisCache.mget(keys, { namespace: 'performance' });
      const getTime = Date.now() - getStartTime;

      expect(results).toHaveLength(batchSize);
      expect(results[0]).toEqual(entries['batch:item:0']);
      expect(results[batchSize - 1]).toEqual(entries[`batch:item:${batchSize - 1}`]);

      // Performance should be reasonable (adjust thresholds as needed)
      expect(setTime).toBeLessThan(5000); // 5 seconds
      expect(getTime).toBeLessThan(2000);  // 2 seconds

      console.log(`Batch operations: Set ${batchSize} items in ${setTime}ms, Get in ${getTime}ms`);
    });
    ...
    skipIfNoRedis()("should handle compression for large objects", async () => {
      const largeObject = {
        ...
        metadata: {
          size: 'large',
          compressed: true,
          arrays: new Array(1000).fill(0).map((_, i) => ({ index: i, value: `item-${i}` }))
        }
      };
      ...
      console.log(`Large object operations: Set in ${setTime}ms, Get in ${getTime}ms`);
    });
  });
  ...
  describe("Error Recovery and Edge Cases", () => {
    ...
    skipIfNoRedis()("should handle namespace and tag combinations", async () => {
      ...
      expect(await redisCache.get('item2', { namespace: 'ns2' })).toBe('value2');
    });
  });
